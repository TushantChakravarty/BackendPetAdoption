...
async function sendOtp(details){
  if (!details) {
    ...
    try{
      ...
      const EmailTemplate = Template.register(verificationCode);
      let mailSent = Email.sendMessage2(details.newEmail, EmailTemplate);
      console.log(mailSent);
  
      let updateObj ={}
      updateObj.OTP = verificationCode
          
      return dao.updateOTP(query,updateObj)
      .then((userUpdated)=>{
        if(userUpdated)
        {
         console.log('updated')
            ...
            return dao
              ...
              .then((userUpdated) => {
                if (userUpdated) {
                  ...
                  updateObj.isEmailVerified = userUpdated.isEmailVerified
                  return mapper.responseMappingWithData(
                    usrConst.CODE.Success,
                    usrConst.MESSAGE.Success,
                    updateObj
 ...
 */
async function setNewPassword(details) {
  if (!details.emailId || !details.password) {
    ...
    if (isUserExists) {
      let newPass = await appUtils.convertPass(details.password);
